...
public class ItemsProvider extends ContentProvider {
	...
	private static final int FLIGHTS_ID = 3;
	...
	private static UriMatcher buildUriMatcher() {
		...
		matcher.addURI(authority, "flights", FLIGHTS);
		matcher.addURI(authority, "flights/#", FLIGHTS_ID);
		return matcher;
	}
	...
	public String getType(Uri uri) {
		...
		switch (match) {
			...
			case FLIGHTS:
				return ItemsContract.Flights.CONTENT_TYPE;
			case FLIGHTS_ID:
				return ItemsContract.Flights.CONTENT_ITEM_TYPE;
			default:
	...
	private SelectionBuilder buildSelection(Uri uri, int match, SelectionBuilder builder) {
		...
		switch (match) {
			...
			case FLIGHTS: {
				return builder.table(Tables.FLIGHTS);
			}
			case FLIGHTS_ID: {
				final String _id = paths.get(1);
				return builder.table(Tables.FLIGHTS).where(ItemsContract.Flights._ID + "=?", _id);
			}
			default: {
				throw new UnsupportedOperationException("Unknown uri: " + uri);
			}
